...
for m in olora inflora sketchlora treelora; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_gpu2.log
    python main.py --config exps/review/stream_smoke/cifar224_${m}.json > run_logs/stream_smoke/cifar224_${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_gpu2.log
done
