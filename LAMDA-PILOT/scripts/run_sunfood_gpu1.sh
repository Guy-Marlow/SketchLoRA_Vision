...
for m in sun397_250mb_olora sun397_250mb_treelora food101_250mb_rainbowprompt; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_sf_gpu1.log
    python main.py --config exps/review/stream_smoke/${m}.json > run_logs/stream_smoke/${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_sf_gpu1.log
done
