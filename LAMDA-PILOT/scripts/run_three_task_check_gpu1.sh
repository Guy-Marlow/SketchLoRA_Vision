...
for m in cifar224_seqlora cifar224_olora cifar224_inflora cifar224_sketchlora cifar224_treelora cifar224_rainbowprompt cifar224_progprompt cifar224_ease cifar224_tuna imagenetr_seqlora imagenetr_olora imagenetr_inflora; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_ttc_gpu1.log
    python main.py --config exps/review/three_task_check/${m}.json > run_logs/stream_smoke/three_task_check_${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_ttc_gpu1.log
done
