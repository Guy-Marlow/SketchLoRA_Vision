...
for m in sun397_250mb_rainbowprompt food101_250mb_inflora food101_250mb_sketchlora; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_sf_gpu4.log
    python main.py --config exps/review/stream_smoke/${m}.json > run_logs/stream_smoke/${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_sf_gpu4.log
done
