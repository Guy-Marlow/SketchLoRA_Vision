...
for m in sun397_500mb_rainbowprompt food101_500mb_inflora food101_500mb_sketchlora; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_sf500_gpu4.log
    python main.py --config exps/review/stream_smoke/${m}.json > run_logs/stream_smoke/${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_sf500_gpu4.log
done
