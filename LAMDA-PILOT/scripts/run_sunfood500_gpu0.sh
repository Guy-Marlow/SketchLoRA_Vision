...
for m in sun397_500mb_seqlora sun397_500mb_sketchlora food101_500mb_treelora; do
    echo "=== starting $m ===" >> run_logs/stream_smoke/queue_sf500_gpu0.log
    python main.py --config exps/review/stream_smoke/${m}.json > run_logs/stream_smoke/${m}.out 2>&1
    echo "=== finished $m (exit $?) ===" >> run_logs/stream_smoke/queue_sf500_gpu0.log
done
