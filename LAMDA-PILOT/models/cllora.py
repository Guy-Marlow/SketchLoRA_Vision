...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self.args.get("boundary_mode") == "budget":
            # proxy_fc (task-local head) is normally sized from the config's fixed
            # init_cls/increment -- under budget mode the real NEW-class count per
            # chunk varies, so override before OurNet.update_fc builds proxy_fc from
            # self.init_cls/self.inc (same fix as models/ease.py, this is its fork).
            # total-known (NOT ce_range()'s hi-lo): proxy_fc is a discrete per-task
            # head slot covering only THIS chunk's new classes -- any carryover-tail
            # class in the chunk's raw data belongs to an earlier slot and must be
            # excluded from this loss (see _init_train's ignore_index masking below),
            # not folded into this slot's width.
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # ce_range()): a carryover-tail sample belongs to an EARLIER,
                # already-existing slot, not this one -- excluded (-1), not remapped.
                ...
                if self._cur_task > 0:
                    ...
                    # -graph autograd error ("modified by an inplace operation") the
                    # reference's original ordering (CE forward computed before the KD
                    # step, .backward() called after) hits the moment shared parameters
                    # are touched by both phases in the same iteration -- confirmed live
                    # (RuntimeError at cur_task=1's first KD-eligible batch, 2026-07-16).
                    kd_ratio = 5.
                    Temperature = 2
                ...
                # CE forward computed here (fresh, post-KD-update parameters if
                # cur_task>0; identical to the reference's own single forward when
                # cur_task==0, since the KD block above is a no-op then).
                output = self._network(inputs, test=False)
                logits = output["logits"]
                # ignore_index=-1 matches the masking sentinel above -- without it, a
